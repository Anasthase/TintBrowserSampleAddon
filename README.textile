...
Asking something to the user is made in two steps. First an addon must create a specific action in a callback. When the user answer, a method in the addon is called to provide user answer. In order to keep track of questions, each action allowing to ask something to the user provide an id (accessible through ??getId()?? on the action). This id will specified when the answer method is called.

Id's are Java ??UUID??. In the answer method, they are given as a ??String??, which can be converted back to ??UUID?? using ??UUID.fromString()??.
...
Constructors:
* ??AskUserConfirmationAction(String title, String message, String positiveButtonCaption, String negativeButtonCaption)??
...
Constructors:
* ??AskUserInputAction(String title, String message)??
** ??title??: Title of the message box.
** ??message??: Message of the message box. Can be ??null??.
* ??AskUserInputAction(String title, String message, String inputHint)??
** ??inputHint??: Hint to show in the EditText view of the dialog. Can be ??null??.
* ??AskUserInputAction(String title, String message, String inputHint, String defaultInput)??
** ??defaultInput??: Default text in the EditText view of the dialog. Can be ??null??.
* ??AskUserInputAction(String title, String message, String inputHint, String defaultInput, int inputType)??
...
Constructors:
* ??AskUserChoiceAction(String title, List&lt;String&gt; choices)??
** ??title??: Title of the message box.
